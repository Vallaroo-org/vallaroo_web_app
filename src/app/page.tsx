...
const DiscoverContent = () => {
  ...
  // Read active tab from URL (synced with LocationFilter)
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background transition-colors duration-300">
      ...
      <main className="flex-1 container mx-auto px-4 py-8 max-w-screen-2xl">
        <div className="mb-6">
          {/* Compact Header - Only show when searching */}
          {isSearching && (
            <div className="mb-4">
              <h1 className="text-2xl font-bold tracking-tight text-foreground">{t('searchResults') || 'Search Results'}</h1>
              <p className="text-sm text-muted-foreground">
                {`${t('resultsFor') || 'Results for'} "${searchQuery}"`}
              </p>
            </div>
          )}

          {/* Ad Banner - Single focused banner */}
          {!isSearching && <AdBanner placement="home_top_carousel" className="h-40 md:h-56 mb-4 rounded-xl overflow-hidden" />}

          {!isSearching && <FollowingStories />}
        </div>
